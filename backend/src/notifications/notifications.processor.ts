...
export class NotificationsProcessor extends WorkerHost {
  ...
  private async handleDocumentShare(
    job: Job<DocumentSharePayload>,
  ): Promise<void> {
    ...
    if (!userId) {
      this.logger.warn(
        `Doc share job ${job.id} has no target user (${invitationId})`,
      );
      return;
    }
      ...
      await this.prisma.notification.create({
        data: {
          ...
          payload: {
            ...
            workspaceName:
              data.workspaceName ?? invitation.document.workspace.name,
            invitedByName:
              data.invitedByName ?? invitation.invitedBy.displayName,

...
export class AssetsService implements OnModuleInit {
	...
	async presign(workspaceId: string, userId: string, dto: PresignAssetDto) {
		...
		const uploadUrl = await this.client.presignedPutObject(this.bucket, objectKey, 60 * 10);
		return {
			objectKey,
			uploadUrl: this.publicizeUrl(uploadUrl),
			bucket: this.bucket,
		};
	}
	...
	private publicizeUrl(url: string): string {
		...
		const internal = this.config.get('MINIO_ENDPOINT', 'localhost');
		const port = this.config.get('MINIO_PORT', '9000');
		const publicPort = this.config.get('MINIO_PUBLIC_PORT', port);
		return url

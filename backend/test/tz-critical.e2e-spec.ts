...
describe('TZ critical flows (e2e)', () => {
	...
	beforeAll(async () => {
		...
		process.env.COLLAB_CMD_CHANNEL = process.env.COLLAB_CMD_CHANNEL ?? 'e2e-collab:cmd';
		process.env.COLLAB_REPLY_CHANNEL = process.env.COLLAB_REPLY_CHANNEL ?? 'e2e-collab:reply';
		process.env.REVALIDATE_SECRET = process.env.REVALIDATE_SECRET ?? 'change-me-revalidate';
	...
	async function createDocument(token: string, workspaceId: string, title: string) {
		const res = await request(app.getHttpServer())
	...
	it('two clients edit one document concurrently — both changes survive', async () => {
		...
		const document = await createDocument(owner.tokens.accessToken, workspace.id, 'Collab doc');
		...
		const idsA = (
			a.ydoc.getArray('blocks').toJSON() as Array<{
				id: string;
			}>
		).map((row) => row.id);
		const idsB = (
			b.ydoc.getArray('blocks').toJSON() as Array<{
				id: string;
			}>
		).map((row) => row.id);
		...
		const blocks = projection.blocksJson as Array<{
			id: string;
			text: string;
		}>;
	...
	it('publish → public payload reflects live edits (SSR/ISR source) + revalidate job', async () => {
		...
		const document = await createDocument(owner.tokens.accessToken, workspace.id, 'Draft');
		...
		expect(
			revalidateHits.some((hit) => hit.slug === published.body.publicSlug || !!hit.tag),
		).toBe(true);
		mockServer.close();

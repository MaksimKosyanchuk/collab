      ...
      {open ? (
        <div className="mt-2 space-y-3 rounded-xl border border-line bg-white/70 p-3">
          {threads.length === 0 ? (
            ...
            threads.map((thread) => (
              <div key={thread.id} className="space-y-2 border-b border-line pb-2 last:border-0">
                    ...
                    <input
                      ...
                      onKeyDown={(event) => {
                        ...
                        startTransition(async () => {
                          const result = await addCommentAction(thread.id, value);
                          if (!result.ok) {
              ...
              <button
                ...
                onClick={() =>
                  startTransition(async () => {
                    ...
                    if (res.ok) {
                      const all = (await res.json()) as Thread[];
                      setThreads(all.filter((row) => row.blockId === blockId));
                    }
                  })
                }
              >

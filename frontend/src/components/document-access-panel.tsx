				...
				<>
					<div>
						...
						<p className="mt-0.5 text-[12px] text-muted">
							Document permission overrides workspace role for this page only (e.g.
							Viewer→Edit or Editor→View). Owner/Admin stay full access. Outsiders get
							access without joining the workspace. Invite → Accept in Alerts.
						</p>
										...
										<span className="flex shrink-0 items-center gap-2">
											<select
												...
												onChange={(event) => {
													...
													run(async () => {
														const result =
															await updateDocumentShareAccessAction(
																workspaceId,
																documentId,
					...
					<div>
						...
						<p className="mt-1 text-xs text-muted">
							Anyone with the URL can open it (no login). Access is view or edit by
							link type — separate from `/p/…` publish.
						</p>
						<select
									...
									>
										...
										<span className="flex shrink-0 items-center gap-2">
											<select
												...
												value={
													link.access === 'MANAGE' ? 'EDIT' : link.access
												}
												disabled={pending}
												onChange={(event) => {
													const access = event.target
													...
													run(async () => {
														const result =
															await updatePublicLinkAccessAction(
																workspaceId,
																documentId,
																link.id,
																access,
														...
														setDoc((prev) => ({
															...prev,
															publicLinks: prev.publicLinks.map(
																(row) =>
																	row.id === link.id
																		? {
																				...row,

...
function labelFor(item: NotificationItem) {
  if (item.type === 'INVITE') {
    ...
    return by
      ? `${by} invited you to ${workspace}${role}`
      : `Invite to ${workspace}${role}`;
  }
  if (item.type === 'DOCUMENT_SHARED') {
    ...
    return by
      ? `${by} shared “${title}” with you${access}`
      : `Shared page “${title}”${access}`;
  }
...
export function NotificationsBell() {
  ...
  function respondShare(
    ...
    setError(null);
    startTransition(async () => {
      const result = await respondDocumentShareInviteAction(
        invitationId,
        action,
      );
      if (!result.ok) {
      ...
      if (action === 'accept') {
        setOpen(false);
        router.push(
          `/app/w/${result.data.workspaceId}/d/${result.data.documentId}`,
        );
        router.refresh();
      }
    });
                    ...
                    !item.readAt ? (
                      <>
                        ...
                        <button
                          ...
                          disabled={pending}
                          onClick={() => respondWorkspace(item, 'decline')}
                        >
                          Decline
                        </button>
                      </>

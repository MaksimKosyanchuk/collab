...
test.describe('TZ: publish → public SSR/ISR page', () => {
	test('/p/[slug] renders published content and refreshes after edit', async ({
		...
		await expect(page.getByText(/Published from the live document|Published\./)).toBeVisible({
			timeout: 15000,
		});
		const published = await publishDocument(request, owner.tokens.accessToken, document.id);
		...
		await expect(publicPage.getByRole('heading', { level: 1 })).toHaveText('SSR Live Title');
		await expect(publicPage.getByText('Hello from published SSR page')).toBeVisible();
		...
		await publicPage.goto(`/p/${slug}`);
		await expect(publicPage.getByRole('heading', { level: 1 })).toHaveText('SSR After Edit');
		...
		const json = (await apiDoc.json()) as {
			title: string;
		};
